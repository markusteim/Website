...
var canvas = document.getElementById("canvasGraphs");
...
var colorbtn1 = document.getElementById("maincolorbtn");
var colorbtn2 = document.getElementById("subcolorbtn1");
var colorbtn3 = document.getElementById("subcolorbtn2");
var barchrtbtn = document.getElementById("barchartbtn");
var linechrtbtn = document.getElementById("linechartbtn");
var piechrtbtn = document.getElementById("piechartbtn");
...
function pieChart(colors, xValue, yValue) {
    ...
    for (var i = 0; i < dataValues.length; i++){
        ...
        // adds the legend
        context.fillRect( 40 , 30 * (i+1) , 20, 20 );  
        context.font = "15px Overpass";
        context.fillText( dataValues[ i ][ 0 ] + " (" + dataValues[ i ][ 1 ] + ")", 125, 30 * (i+1) + 15 );
    }

    context.font = "25px Overpass";
    context.textAlign = "center";
    context.fillStyle = "black"; 
    context.fillText("Markus' activities during a busy week", x, yValue - 30);
}

function barChart(colors, days, x, y){
    
    var dataValues = [["Studying", 9], ["Playing Games" , 2], ["Exercise" , 0], ["Studying", 7], ["Playing Games" , 1], ["Exercise" , 0], ["Studying", 10], ["Playing Games" , 2], ["Exercise" , 1], ["Studying", 8], ["Playing Games" , 2], ["Exercise" , 0], ["Studying", 4], ["Playing Games" , 0], ["Exercise" , 1], ["Studying", 5], ["Playing Games" , 1], ["Exercise" , 2], ["Studying", 5], ["Playing Games" , 1], ["Exercise" , 1]];
    var maxHours = 12;
    ...
    for (var i = 0; i < maxHours; i++){
        ...
        context.textAlign = "center";
        context.fillStyle = "black"; 
        context.fillText((i) + "h", startingX - 20, startingY - (i*50));
        //draw background lines
        context.stokeStyle = "rgb(0,0,0)"; 
        context.beginPath();
            context.moveTo(startingX, startingY - (i*50));
            context.lineTo(endingX, startingY - (i*50));
        context.stroke();
    }
    //draw the days
    for (var i = 0; i < days.length; i++){
        context.font = "20px Overpass";
        context.textAlign = "center";
        context.fillStyle = "black"; 
        context.fillText(days[i], (startingX + 95 ) * (i+1), 640);
    }
    //draw the bars
    for (var i = 0; i < dataValues.length; i++){
        ...
        context.fillRect((startingX), startingY - (50 * dataValues[i][1]) , 35, 50*dataValues[i][1] ); 
        colorIncrementer += 1;
        startingX += 50;
    }
    ...
    for (var i = 0; i < 3; i++){
        context.fillStyle = colors[i];
        context.fillRect( endingX + 40 , 30 * (i+1) , 20, 20 );  
        context.font = "15px Overpass";
        context.fillText( dataValues[ i ][ 0 ] , endingX + 120, 30 * (i+1) + 15 );
    }
...
function lineChart(colors, days, x, y){
    ...
    for (var x = 0; x < 3; x++){
        ...
        for (var i = x; i < dataValues.length; i+=3){
            ...
            if (i + 3 > 20) { 
                context.fillRect((xDifference * counter) + 13, (startingY - (dataValues[i][1] * 50)) - 6,15,15);
                break;
            }

            context.fillRect((xDifference * counter) + 13, (startingY - (dataValues[i][1] * 50)) - 6,15,15);
            context.beginPath();
                context.moveTo(xDifference * counter + 20, startingY - (dataValues[i][1] * 50));
                context.lineTo(xDifference * (counter + 1) + 20, startingY - (dataValues[i+3][1] * 50));
            context.stroke();
           
            currentYValue = (dataValues[i][1] * 50);
            counter += 1; 
        }
    }
}
